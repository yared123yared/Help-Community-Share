...
func TestPutProfile(t *testing.T) {
	...
	req, err := http.NewRequest("PUT", "/pharm/profiles/1028", bytes.NewBuffer(jsonStr))
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("Content-Type", "application/json")
	...
	router.PUT("/pharm/profile/:id", profHandler.PutProfile)
	router.ServeHTTP(rr, req)
...
func TestDeleteProfile(t *testing.T) {
	req, err := http.NewRequest("DELETE", "/pharm/profiles/1", nil)
	if err != nil {
		t.Fatal(err)
	}
	ProfReq := PharmacistRepo.NewMockPharmacistProfileGormRepo(nil)
	...
	router.DELETE("/pharm/profile/:id", profHandler.DeleteProfile)
	router.ServeHTTP(rr, req)
